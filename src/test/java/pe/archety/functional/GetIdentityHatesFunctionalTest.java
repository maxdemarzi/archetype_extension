...
public class GetIdentityHatesFunctionalTest {
    ...
    public void shouldGetIdentityHatesByEmail() throws IOException {
        JaxRsResponse response = request.get("service/identity/hates?email=" + validEmail);
    ...
    public void shouldGetIdentityHatesByHash() throws IOException {
        JaxRsResponse response = request.get("service/identity/hates?md5hash=" + validMD5Hash);
        ArrayList actual = objectMapper.readValue(response.getEntity(), ArrayList.class);
        assertEquals(validIdentityPages, actual);
    }
